% SDMAddDataMemoryFrugalValidation() - Adds data for validation for SDM training.
%
% (C)opyright 2016, Ashton Fagg

function data = SDMAddDataMemoryFrugalValidation(imageList, landmarkList, nSynth, model, nFrames)

images = getListOfFiles(imageList);
landmarks = getListOfFiles(landmarkList);

keepIdx = randperm(length(images), nFrames);
images = images(keepIdx);
landmarks = landmarks(keepIdx);
...
function chunkData = doChunk(model, nSynth, images, landmarks, chunkID, noiseParams)
    ...
    for i = 1:numel(images)
        ...
        chunkData.nExamples = chunkData.nExamples + (2*nSynth);
...
function [deltaFrame, featFrame] = addFrame(im, pts, model, nSynth, layer)
    ...
    for example = 1:nSynth
        ...
        %ptsNoise = getPerturbation(pts, reshape(model.mu, [], 2), im);
        ...
        %delta = deltaExtract(model, im, pts, ptsNoise);
        %delta = double(delta);
